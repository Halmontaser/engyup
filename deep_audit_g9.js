...
// 1. Audit components.db for g9
const rows = db.prepare("SELECT id, type, data FROM activities WHERE lesson_id LIKE 'g9%'").all();
const counts = {};
const issues = [];
const missingImages = [];
...
console.log('Total G9 Activities:', rows.length);
...
if (fs.existsSync(imagesDir)) {
    g9Images = fs.readdirSync(imagesDir).filter(f => f.startsWith('g9_')).length;
}
if (fs.existsSync(audioDir)) {
    g9Audios = fs.readdirSync(audioDir).filter(f => f.startsWith('g9_')).length;
}

console.log(`Grade 9 Images found: ${g9Images}`);
console.log(`Grade 9 Audio files found: ${g9Audios}`);
