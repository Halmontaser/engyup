...
const promptSource = 'E:\\Books\\english_images\\english\\Grade 11\\images\\grade 11 visual vocab';
...
for (let key in imageLookup) {
    ...
    let destName = `g11_prompt_${key}${ext}`;
    ...
    mediaMap[`g11-prompt-${key}`] = `/media/images/prompts/${destName}`;
    count++;
}

fs.writeFileSync(mediaMapPath, JSON.stringify(mediaMap, null, 2));
console.log(`Copied and mapped ${count} prompt images for Grade 11.`);
