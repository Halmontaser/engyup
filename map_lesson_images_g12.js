...
const promptSource = 'E:\\Books\\english_images\\english\\Grade 12\\images\\Grade 12 visual vocab';
...
for (let key in imageLookup) {
    ...
    let destName = `g12_prompt_${key}${ext}`;
    ...
    mediaMap[`g12-prompt-${key}`] = `/media/images/prompts/${destName}`;
    count++;
}

fs.writeFileSync(mediaMapPath, JSON.stringify(mediaMap, null, 2));
console.log(`Copied and mapped ${count} prompt images for Grade 12.`);
