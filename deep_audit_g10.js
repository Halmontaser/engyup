...
// 1. Audit components.db for g10
const rows = db.prepare("SELECT id, type, data FROM activities WHERE lesson_id LIKE 'g10%'").all();
const counts = {};
const issues = [];
const missingImages = [];
...
console.log('Total g10 Activities:', rows.length);
...
if (fs.existsSync(imagesDir)) {
    g10Images = fs.readdirSync(imagesDir).filter(f => f.startsWith('g10_')).length;
}
if (fs.existsSync(audioDir)) {
    g10Audios = fs.readdirSync(audioDir).filter(f => f.startsWith('g10_')).length;
}

console.log(`Grade 9 Images found: ${g10Images}`);
console.log(`Grade 9 Audio files found: ${g10Audios}`);
