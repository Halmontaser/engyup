...
// 1. Audit components.db for g12
const rows = db.prepare("SELECT id, type, data FROM activities WHERE lesson_id LIKE 'g12%'").all();
const counts = {};
const issues = [];
const missingImages = [];
...
console.log('Total g12 Activities:', rows.length);
...
if (fs.existsSync(imagesDir)) {
    g12Images = fs.readdirSync(imagesDir).filter(f => f.startsWith('g12_')).length;
}
if (fs.existsSync(audioDir)) {
    g12Audios = fs.readdirSync(audioDir).filter(f => f.startsWith('g12_')).length;
}

console.log(`Grade 9 Images found: ${g12Images}`);
console.log(`Grade 9 Audio files found: ${g12Audios}`);
