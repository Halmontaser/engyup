...
// 1. Get all g9 activities, group by lesson, sorted by sort_order
const rows = db.prepare(`SELECT id, lesson_id, type, sort_order FROM activities WHERE lesson_id LIKE 'g9%' ORDER BY lesson_id, sort_order`).all();
...
// Delete ONLY the images and generic audio (not CS audio) of g9 from the mediaMap before updating
Object.keys(mediaMap).forEach(key => {
    if (key.includes('g9-gen-g9') || key.startsWith('g9-u')) {
        // Keep CS audio logic intact as it is 'g9-x.y-cs-audio'
...
if (fs.existsSync(imagesDir)) {
    const images = fs.readdirSync(imagesDir).filter(f => f.startsWith('g9_') && f.match(/\.(png|jpg|jpeg|webp)$/i));
    for (const file of images) {
        // e.g. g9_u1_l1_10_flsh_act0_i0.png
...
if (fs.existsSync(audioDir)) {
    const audios = fs.readdirSync(audioDir).filter(f => f.startsWith('g9_') && f.endsWith('.mp3'));
    for (const file of audios) {
        // e.g. g9_u1_l1.10_gap_act2_a0.mp3
...
console.log(`Successfully updated media_map.json with g9 media.`);
