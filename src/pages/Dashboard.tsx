...
export function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 flex flex-col md:flex-row" style={{ backgroundImage: 'radial-gradient(at 40% 20%, rgba(30, 64, 175, 0.08) 0px, transparent 50%), radial-gradient(at 80% 0%, rgba(14, 165, 233, 0.06) 0px, transparent 50%), radial-gradient(at 0% 50%, rgba(168, 85, 247, 0.04) 0px, transparent 50%)' }}>
      {/* Mobile Menu Overlay */}
      {isMobileMenuOpen && (
        <div className="fixed inset-0 bg-slate-900/60 z-40 backdrop-blur-sm md:hidden animate-fade-in" onClick={() => setIsMobileMenuOpen(false)} />
      )}
      ...
      <aside className={cn(
        ...
        <div className="p-6 border-b border-slate-200 flex items-center justify-between bg-gradient-to-br from-blue-50/50 to-cyan-50/50">
          <div className="flex items-center gap-2">
            <img src="/logo.png" alt="Engy Up" className="h-10 md:h-14 w-auto drop-shadow-lg" />
          </div>
          <button className="md:hidden text-slate-500 hover:bg-slate-100 p-2 rounded-lg transition-colors" onClick={() => setIsMobileMenuOpen(false)}>
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-4 md:p-6 border-t border-slate-200 bg-gradient-to-t from-slate-50 to-transparent">
          <button onClick={signOut} className="flex items-center gap-3 w-full p-3 md:p-4 text-slate-600 hover:bg-white hover:shadow-md rounded-xl md:rounded-2xl transition-all duration-300 group">
            <div className="w-9 h-9 md:w-10 md:h-10 rounded-xl bg-slate-100 flex items-center justify-center group-hover:bg-red-50 group-hover:text-red-500 transition-colors">
      ...
      {/* Main Content */}
      <main className="flex-1 overflow-y-auto w-full order-2 md:order-1">
        <header className="bg-white/80 backdrop-blur-xl border-b border-slate-200 p-3 md:p-4 lg:p-6 flex items-center justify-between sticky top-0 z-10 shadow-sm">
          <div className="flex items-center gap-2 md:gap-3 flex-1 min-w-0">
            <button className="p-2 -ml-2 text-slate-600 hover:bg-slate-100 rounded-xl transition-colors md:hidden" onClick={() => setIsMobileMenuOpen(true)}>
              <Menu className="w-6 h-6" />
            </button>
            <div className="min-w-0">
              <h2 className="text-xl md:text-2xl lg:text-3xl font-bold text-slate-900 tracking-tight truncate">
                {activeTab === 'dashboard' ? (
                  <span>
              ...
              <div className="flex items-center gap-1.5 md:gap-2 text-slate-500 text-xs mt-1 md:mt-2">
                <div className="w-5 h-5 md:w-6 md:h-6 rounded-lg bg-gradient-to-br from-blue-100 to-cyan-100 flex items-center justify-center flex-shrink-0">
                  <School className="w-2.5 h-2.5 md:w-4 md:h-4 text-blue-700" />
                </div>
                <span className="font-medium truncate">{activeTenant?.name || 'Engy Up English'}</span>
        ...
        <div className="p-3 md:p-6 lg:p-8 max-w-7xl mx-auto space-y-4 md:space-y-6 lg:space-y-8">
          {activeTab === 'dashboard' && (
            <>
              {isLoadingCourses ? (
                <div className="flex justify-center py-16 md:py-20">
                  <Loader2 className="w-10 h-10 md:w-12 md:h-12 text-blue-700 animate-spin" />
                </div>
              ) : courses.length === 0 ? (
                <div className="text-center py-16 md:py-20 px-4">
                  ...
                  <h3 className="text-lg md:text-xl font-bold text-slate-900">No Courses Found</h3>
                  <p className="text-slate-500 mt-2 text-sm md:text-base">Courses will appear here once the curriculum data is migrated.</p>
                </div>
              ) : (
                    ...
                    return (
                      ...
                      >
                        <div className="relative h-40 sm:h-48 w-full overflow-hidden">
                          ...
                          <div className={`absolute top-3 right-3 sm:top-4 sm:right-4 px-2.5 py-1 sm:px-4 sm:py-1.5 rounded-full text-[10px] sm:text-xs font-bold uppercase tracking-wider text-white bg-gradient-to-r ${gradientColors[i % gradientColors.length]} shadow-xl shadow-black/20 backdrop-blur-sm`}>
                            {course.course_code || 'English'}
                          </div>
                        </div>
                        <div className="p-4 sm:p-6 flex flex-col flex-1">
                          <h3 className="text-lg sm:text-xl lg:text-2xl font-bold text-slate-900 mb-1.5 sm:mb-2 group-hover:text-blue-700 transition-colors line-clamp-1">{course.title}</h3>
                          <p className="text-xs sm:text-sm text-slate-500 mb-4 sm:mb-6 line-clamp-2 leading-relaxed flex-1">{course.description || 'Comprehensive English language curriculum designed for excellence.'}</p>
                          ...
                          <div className="flex items-center justify-between text-xs sm:text-sm mb-3 sm:mb-4">
                            ...
                            <span className="font-bold text-blue-700 text-base sm:text-lg">{progressPercent}%</span>
                          </div>

                          <div className="w-full h-2 sm:h-3 bg-slate-100 rounded-full overflow-hidden mb-4 sm:mb-5 shadow-inner">
...
function SidebarItem({ icon: Icon, label, active = false, onClick }: { icon: any; label: string; active?: boolean; onClick?: () => void }) {
  return (
    <button
      onClick={onClick}
      className={cn(
        "flex items-center gap-3 w-full p-3 md:p-4 rounded-xl md:rounded-2xl font-semibold transition-all duration-300 relative overflow-hidden group",
        active
    ...
    >
      ...
      <div className={cn("w-9 h-9 md:w-10 md:h-10 rounded-lg md:rounded-xl flex items-center justify-center transition-all duration-300 relative z-10 flex-shrink-0",
