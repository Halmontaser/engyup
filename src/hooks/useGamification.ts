...
export const useGamification = () => {
  ...
  const checkCourseCompletion = useCallback(async (courseId: string) => {
    ...
    try {
      const { data: lessons } = await supabase.from('modules').select('lessons(id)').eq('course_id', courseId);
      const allLessonIds = lessons?.flatMap(m => m.lessons.map((l: any) => l.id)) || [];
      if (allLessonIds.length === 0) return false;

      const { data: completed } = await supabase
        .from('user_progress').select('lesson_id')
        .eq('user_id', user.id).eq('status', 'completed').in('lesson_id', allLessonIds);
      
      const completedIds = completed?.map(p => p.lesson_id) || [];

      if (completedIds.length === allLessonIds.length) {
        return await trackEvent({ eventType: 'course_completed', entityType: 'course', entityId: courseId, courseId });
      }
      return false;
    } catch (err) {
